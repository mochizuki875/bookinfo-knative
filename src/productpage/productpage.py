...
detailsPort = "details" if (os.environ.get("DETAILS_PORT") is None) else os.environ.get("DETAILS_PORT")
ratingsHostname = "ratings" if (os.environ.get("RATINGS_HOSTNAME") is None) else os.environ.get("RATINGS_HOSTNAME")
ratingsPort = "ratings" if (os.environ.get("RATINGS_PORT") is None) else os.environ.get("RATINGS_PORT")
reviewsHostname = "reviews" if (os.environ.get("REVIEWS_HOSTNAME") is None) else os.environ.get("REVIEWS_HOSTNAME")
reviewsPort = "reviews" if (os.environ.get("REVIEWS_PORT") is None) else os.environ.get("REVIEWS_PORT")

flood_factor = 0 if (os.environ.get("FLOOD_FACTOR") is None) else int(os.environ.get("FLOOD_FACTOR"))

# リクエストタイムアウト値を環境変数から取得し設定
timeoutSec = 3.0 if (os.environ.get("TIMEOUT") is None) else float(os.environ.get("TIMEOUT"))
...
def getProductDetails(product_id, headers):
    try:
        ...
        # タイムアウト値設定
        res = requests.get(url, headers=headers, timeout=timeoutSec)
